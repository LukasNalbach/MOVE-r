...
class avl_tree {
    ...
    inline avl_node<T>* build_subtree(idx_t l, idx_t r, std::function<avl_node<T>*(idx_t)> &at, uint16_t max_tasks = 1) {
        if (r == l) {
            avl_node<T>* n_l = at(l);
            n_l->rc = n_l->lc = NULL;
            n_l->h = 0;
     ...
     */
    template <typename idx_t>
    void insert_array(idx_t l, idx_t r, std::function<avl_node<T>*(idx_t)> &at, uint16_t max_tasks = 1) {
        if (empty() && l >= 0 && r >= l) {
